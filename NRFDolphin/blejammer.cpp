...
void updateOLED() {
  ...
  u8g2.print(currentMode == WiFi_MODULE ? "Wi-Fi" : 
          currentMode == BLE_MODULE ? "BLE" : 
          currentMode == Bluetooth_MODULE ? "Bluetooth" : 
          currentMode == RC_MODULE ? "RC Drones" : 
          "OFF"); 
...
void checkModeChange() {
  if (modeChangeRequested) {
    ...
    currentMode = static_cast<OperationMode>((currentMode + 1) % 5);
    initializeRadios();
    updateOLED();
  }
}
...
void blejammerLoop() {
  ...
  if (currentMode == BLE_MODULE) {
    ...
    if (now - lastChange >= jammingInterval) {
      int channel = wifi_channels[index];
      radio1.setChannel(channel);
      radio2.setChannel(channel);
      radio3.setChannel(channel);
      ...
      index = (index + 1) % (sizeof(wifi_channels) / sizeof(wifi_channels[0]));
      lastChange = now;
    } 
