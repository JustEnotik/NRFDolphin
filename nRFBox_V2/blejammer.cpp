...
void updateOLED() {
  ...
  if(currentMode == WiFi_MODULE || currentMode == BLE_MODULE || currentMode == Bluetooth_MODULE) {
    u8g2.drawXBMP(75, 9, 51, 64, dolphin_think);
  } else {
    u8g2.drawXBMP(73, 14, 49, 54, dolphin_cry);
  }
...
void blejammerLoop() {
  ...
  if (currentMode == DEACTIVE_MODE) {
    ...
    pixels.setPixelColor(0, pixels.Color(64, 0, 0));  // активный джамминг — красный
  }
  pixels.show();
}
