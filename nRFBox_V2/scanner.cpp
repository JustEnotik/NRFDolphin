...
void scannerLoop() {
  ...
  outputChannels();  // отрисовывает и сбрасывает channel[]

  // если есть сигналы (любые пакеты) — красный, иначе белый
  if (norm > 0) {
    pixels.setPixelColor(0, pixels.Color(64, 0, 0));  // есть активность
  } else {
    pixels.setPixelColor(0, pixels.Color(8, 8, 8));  // тишина
  }
